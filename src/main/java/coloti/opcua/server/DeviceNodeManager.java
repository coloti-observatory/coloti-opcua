...
import coloti.opcua.ConsumerLibrary;
import coloti.opcua.ServerNamespace;
...
public class DeviceNodeManager {
    ...
    private ServerItem createServerItem(String itemId, String datatype, boolean b, Consumer<DataValue> consumer,
            UaFolderNode folder) {
        ServerItem item = new ServerItem(itemId, datatype, b,namespace.getNodeCxt(), namespace.getNamespaceIdx(), folder);
    ...
    private ServerItem createServerItem(AstrimaIcdDataPoint dp, UaFolderNode folder) {
        ServerItem item = new ServerItem(dp.getOpcUaNode().split(";")[1].split("=")[1], dp.getOpcUaDataType(), dp.isWritable(),namespace.getNodeCxt(), namespace.getNamespaceIdx(), folder);
        System.out.println(dp.toString());
        if(dp.isWritable()){
            item.addWriteListener(ConsumerLibrary.consumerMap.get(dp.getShortName()));
        }
        else{
            item.setNodefeeder(ConsumerLibrary.monitorMap.get(dp.getShortName()));
        }
        String dataType = dp.getOpcUaDataType().toUpperCase();

        if(dataType.equals("DOUBLE")){
            System.out.println("Default value: "+dp.getDefaultValue());
            item.updateValue(Double.parseDouble(dp.getDefaultValue()));
        }
        else if(dataType.equals("INT32")){
            System.out.println("Default value: "+dp.getDefaultValue());
            item.updateValue((int) Double.parseDouble(dp.getDefaultValue()));
        }
            ...
            System.out.println("Default value: "+dp.getDefaultValue());
            item.updateValue(Float.parseFloat(dp.getDefaultValue()));
        }
        else{
    ...
    public static void main(String[] args) {
        DeviceNodeManager dnm = new DeviceNodeManager(null,
                "/home/coloti/OPCUA/StructuredOpcua/opcuaserver/MCS_ICD_COLOTI.xlsx");
